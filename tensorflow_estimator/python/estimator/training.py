...
class _TrainingExecutor(object):
  ...
  def _start_distributed_training(self, saving_listeners=None):
    ...
    while True:
      ...
      # the input fn MUST take ONLY 3 arguments: "subset" (for train or test data), the per-worker "batch-size"
      # and the location of input data "data_dir"
      ...
      updated_input_fn = functools.partial(switched_input_fn, config.get_datadir, subset='train',
                                           batch_size=node_batch_size)
      loss, should_switch_input_fn = self._estimator.train(input_fn=updated_input_fn,
                                                           max_steps=self._train_spec.max_steps,
      ...
      logging.info('@sahiltyagi4 start time on switch input fn ' + str(start_time) +
                   ' and end time on switch input fn ' + str(time.time()))

      #update "batch_size_list" parameter of TF_CONFIG to accomodate new per-worker batchsizes.
      #NOTE: global batchsize 'B' remains constant in this case as workers aren't added/removed
      batchsize_conf = open(os.path.join(config.model_dir, 'clusterbatchsizes.conf'), 'r')
      updated_batchsize_list = str(batchsize_conf.readline().strip())
      parse_json = json.loads(os.environ['TF_CONFIG'])
      parse_json['batch_size_list'] = updated_batchsize_list
      os.environ['TF_CONFIG'] = json.dumps(parse_json)

      if not should_switch_input_fn:
        logging.info('Loss for final step: %s.', loss)
        break

    logging.info('training terminated for good...either because steps/epochs specified are complete or '
                 + 'objective threshold is reached')
